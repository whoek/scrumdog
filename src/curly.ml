...
module Error = struct
  ...
  let no_curl () =   (* scrumdog *)
    Printf.printf "%s%!" "Issue accessing 'curl'\n";
    Printf.printf "%s%!" "Check if you can run 'curl' from command line\n";
    Printf.printf "%s%!" "Please report an issue to willem@matimba.com\n";
    Printf.printf "%s%!" "Provide error message below in your email.\n\n"
    (* exit 1 *)

  let pp fmt = function
    | Bad_exit p ->
      let _ = no_curl () in    (* scrumdog *)
...
let run prog args stdin_str =
  (* let () = Format.printf "DEBUG prog:%s \n"  prog in *)
  (* let () = Format.printf "DEBUG stdin_str:%s \n"  stdin_str in *)
  let (stdout, stdin, stderr) =
    let prog =
      prog :: (List.map Filename.quote args)
      |> String.concat " " in
    (* let () = Printf.printf "DEBUG prog:%s\n" prog in *)
...
let run ?(exe="curl") ?(args=[]) req =    (* calling curl *)
